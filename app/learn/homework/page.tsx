...
export default function Homework() {
  const t = useT();
  return (
    <Shell role={learnerRole} nav={learnerNav} title={t3("拍照作业", "Photo homework", "Kazi ya picha")} sub={t3("手写练习 · 识别 → 双智能体 批改 → 老师签发", "Handwritten practice · recognition → two-agent grading → teacher sign-off", "Mazoezi ya mwandiko · utambuzi → usahihishaji wa wakala wawili → idhini ya mwalimu")} net={2}>
      <div className="grid c3">
        ...
        <Panel className="in in-2" lift={false}><Head title={t3("批改结果", "Feedback", "Maoni")} right={<Badge tone="warn">{t(t3("待老师签发", "Awaiting teacher sign-off", "Inasubiri idhini ya mwalimu"))}</Badge>} />
          <ul className="list">
            ...
            <li><Badge tone="warn">!</Badge><div className="t"><b>{t(t3("书写：「个」结构偏右，「们」的「门」框略小", "Writing: 个 leans right; the 门 in 们 is a little small", "Kuandika: 个 imeinama kulia; 门 ndani ya 们 ni ndogo kidogo"))}</b><span>{t(t3("书写智能体 · 间架结构校验", "Writing Agent · structure check", "Wakala wa Kuandika · ukaguzi wa muundo"))}</span></div></li>
            ...
            <li><Badge>?</Badge><div className="t"><b>{t(t3("「很高兴」是否需要「都」——两个智能体判断不一致", "Whether 很高兴 needs 都: the two agents disagree", "Ikiwa 很高兴 inahitaji 都: wakala wawili hawakubaliani"))}</b><span>{t(UI.abstain)}</span></div></li>
          </ul>
          <div style={{ marginTop: 14 }}><div className="eyebrow" style={{ marginBottom: 8 }}>{t(t3("这次批改是怎么来的", "How this feedback was produced", "Maoni haya yalitengenezwa vipi"))}</div><Trace steps={[["route", "OCR"], ["ground", t(t3("课程图谱 Unit 5", "Course graph Unit 5", "Grafu ya kozi Unit 5"))], ["verify", t(t3("评分智能体", "Grader", "Msahihishaji"))], ["verify", t(t3("复核智能体", "Verifier", "Mkaguzi"))], ["human", t(t3("王老师签发", "Wang Laoshi signs", "Wang Laoshi anaidhinisha"))]]} /></div>
        </Panel>
