    ...
    <Shell role={opsRole} nav={opsNav} title={t3("语言包管理", "Language pack management", "Usimamizi wa vifurushi vya lugha")} sub={t3("界面语言：设置、审核、修正、发布；机器草稿 → 母语审校 → 发布，术语表统一口径", "Interface languages: configure, review, correct, publish; machine draft → native review → publish, with a shared glossary", "Lugha za kiolesura: sanidi, kagua, sahihisha, chapisha")} net={3}>
      <div className="grid c4">
        ...
        <Panel className="span2 in in-2"><Head title={t3("语言与覆盖率", "Languages & coverage", "Lugha na ufikiaji")} right={<button className="btn btn--sm"><LanguagesIcon size={14} /> {t(t3("新增语言", "Add language", "Ongeza lugha"))}</button>} />
          <div className="tbl"><table><thead><tr><th>{t(t3("语言", "Language", "Lugha"))}</th><th>{t(t3("覆盖率", "Coverage", "Ufikiaji"))}</th><th>{t(t3("状态", "Status", "Hali"))}</th><th>{t(t3("回退", "Fallback", "Mbadala"))}</th><th></th></tr></thead><tbody>
            {LANGS.map((l) => <tr key={l.code} style={{ background: l.code === lang ? "rgba(108,140,255,.08)" : undefined }}><td><b>{l.label}</b> <span className="dim small">{l.code}</span></td><td style={{ minWidth: 160 }}><Bar v={coverage[l.code]} tone={coverage[l.code] > .95 ? "good" : coverage[l.code] > .5 ? "warn" : ""} /><span className="small mute">{Math.round(coverage[l.code] * 100)}%</span></td><td><Badge tone={status[l.code][1]}>{status[l.code][0]}</Badge></td><td className="small mute">{l.code === "en" ? "—" : "English"}</td><td><button className="btn btn--sm btn--ghost" onClick={() => setLang(l.code)}>{t(t3("管理", "Manage", "Dhibiti"))}</button></td></tr>)}
          </tbody></table></div>
        </Panel>
        ...
        <Panel className="span3 in in-4" lift={false}>
          ...
          {tab === "glossary" && (
            <div className="tbl"><table><thead><tr><th>{t(t3("术语", "Term", "Istilahi"))}</th><th>中文</th><th>English</th><th>Kiswahili</th><th>Français</th><th>{t(t3("规则", "Rule", "Kanuni"))}</th></tr></thead><tbody>
              {[["HanQiao", "汉桥", "HanQiao", "HanQiao", "HanQiao", t(t3("品牌名不译", "Brand name, never translated", "Jina la chapa, halitafsiriwi"))], ["TIDAR KAG", "TIDAR KAG", "TIDAR KAG", "TIDAR KAG", "TIDAR KAG", t(t3("技术名不译", "Technical name, not translated", "Jina la kiufundi"))], ["Study Companion", "伴学 Agent", "Study Companion", "Rafiki wa Masomo", "Compagnon d'étude", t(t3("Agent 名称按语言意译", "Agent names are localised", "Majina ya mawakala hutafsiriwa"))], ["Teacher sign-off", "老师签发", "Teacher sign-off", "Idhini ya mwalimu", "Validation de l'enseignant", t(t3("强调「老师做主」", "Keeps the teacher-in-charge meaning", "Huweka maana ya mwalimu kuongoza"))], ["HSK / YCT", "HSK / YCT", "HSK / YCT", "HSK / YCT", "HSK / YCT", t(t3("考试名不译", "Exam names, not translated", "Majina ya mitihani"))], ["Preview environment", "演示环境", "Preview environment", "Mazingira ya onyesho", "Environnement de démonstration", t(t3("全站统一", "Site-wide, one wording", "Neno moja kote"))]].map((r) => <tr key={r[0]}>{r.map((c, i) => <td key={i} className={i === 5 ? "small mute" : ""}>{i === 0 ? <b>{c}</b> : c}</td>)}</tr>)}
            </tbody></table></div>
          )}
          {tab === "workflow" && (
            <ul className="list">
