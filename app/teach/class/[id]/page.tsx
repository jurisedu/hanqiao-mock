    ...
    <Shell role={teacherRole} nav={teacherNav} title={c.name} sub={t3(`${c.school} · ${c.city} · ${c.n} 人 · 下次直播 ${t(c.next)}`, `${c.school} · ${c.city} · ${c.n} learners · next live ${t(c.next)}`, `${c.school} · ${c.city} · wanafunzi ${c.n}`)} net={3}>
      <div className="grid c4">
        ...
        <Panel className="span2 in in-3"><Head title={t3("班级薄弱点 · 校准", "Class weak points · calibrated", "Sehemu dhaifu za darasa")} />
          ...
          <div className="row" style={{ marginTop: 10 }}><button className="btn btn--sm btn--primary">{t(t3("让评测 Agent 组一份「第三声」小测", "Ask the Assessment Agent to build a tone-3 quiz", "Omba Wakala wa Tathmini aunde jaribio la toni ya 3"))}</button><span className="dim small">→ {t(t3("进入提议审批", "goes to proposal approval", "huenda kwenye idhini"))}</span></div>
        </Panel>
