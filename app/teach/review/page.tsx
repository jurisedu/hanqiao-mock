        ...
        <Panel className="span2 in in-2" glow lift={false}>
          {allDone ? (
            <div style={{ textAlign: "center", padding: "48px 20px" }}>
              ...
              <p className="mute" style={{ marginTop: 6 }}>{t(t3("今天的分歧项、低置信与主观题都已由你裁定。裁定已写入创作链，并作为评测样本改进 Agent。", "Every disagreement, low-confidence and subjective item is decided. Your calls are written to the creation chain and improve the agents.", "Kila kitu kimeamuliwa; maamuzi yako yanaboresha mawakala."))}</p>
          ...
          <div className="grid c2" style={{ marginTop: 18 }}>
            <div className="panel"><div className="eyebrow">{t(t3("评分 Agent", "Grader agent", "Wakala wa alama"))}</div><div style={{ fontSize: 30, fontWeight: 800, fontFamily: "var(--serif)" }}>{sel.kind === "decide" ? "62" : sel.kind === "sign" ? "88" : "—"}</div><p className="small mute">{sel.kind === "decide" ? t(t3("「很」第三声起调偏高，判为二声倾向", "很 starts high, leaning toward tone 2", "很 huanza juu, inaelekea toni ya 2")) : t(sel.ai)}</p></div>
